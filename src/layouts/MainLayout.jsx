...
import React, { useState, useEffect } from 'react'; // useEffect is used
import { Outlet, Link, NavLink } from 'react-router-dom'; // Added NavLink
...
import SmithieMessageModal from '../components/SmithieMessageModal.jsx'; // <<< NEW: Import Smithie's modal
...
function MainLayout() {
  ...
  // <<< NEW: State and handlers for Smithie's Message Modal >>>
  ...
  return (
    <>
            ...
            <div className="flex justify-between items-center">
              <div className="flex items-center gap-2 sm:gap-3 flex-shrink-0">
                ...
                <img 
                  ...
                  onClick={() => { // <<< MODIFIED: Open Smithie Modal instead of alert >>>
                    triggerPetExcitement();
                    openSmithieModal(); 
                  }}
                />
                <div className="flex flex-col">
                  <Link to="/" onClick={handleDesktopNavClick}>
                    <h1 className="text-lg sm:text-xl md:text-2xl font-bold text-purple-800 tracking-wide cursor-pointer hover:text-purple-600 transition-colors" onMouseEnter={triggerPetExcitement}>
              ...
              <div className="flex items-center space-x-1 sm:space-x-2">
                <div className="w-12 h-12 sm:w-14 sm:h-14 md:w-16 md:h-16 cursor-pointer transform hover:scale-110 transition-transform duration-300" onClick={triggerDogOnly} onMouseEnter={triggerDogOnly}><Lottie animationData={dogAnimation} loop={isDogActive} autoplay={isDogActive} className="w-full h-full" /></div>
                <div className="w-12 h-12 sm:w-14 sm:h-14 md:w-16 md:h-16 cursor-pointer transform hover:scale-110 transition-transform duration-300" onClick={triggerCatOnly} onMouseEnter={triggerCatOnly}><Lottie animationData={catAnimation} loop={isCatActive} autoplay={isCatActive} className="w-full h-full" /></div>
              </div>
            </div>
      ...
      {/* Render General Enquiry Modal */}
      <GeneralEnquiryModal 
        isOpen={isEnquiryModalOpen} 
        onClose={closeEnquiryModal} 
      />
      
      {/* <<< NEW: Render Smithie Message Modal >>> */}
      <SmithieMessageModal 
